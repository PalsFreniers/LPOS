...
{
    return DiskReadSector(disk, 0, 1, g_Data->BS.BootSectorBytes);
}

bool FAT_ReadFat(DISK* disk)
{
    return DiskReadSector(disk, g_Data->BS.BootSector.ReservedSectors, g_Data->BS.BootSector.SectorsPerFat, g_Fat);
}
...
bool FAT_Initialize(DISK* disk)
{
    ...
    if (!DiskReadSector(disk, rootDirLba, 1, g_Data->RootDirectory.Buffer))
    {
...
FAT_File* FAT_OpenEntry(DISK* disk, FAT_DirectoryEntry* entry)
{
    ...
    if (!DiskReadSector(disk, FAT_ClusterToLba(fd->CurrentCluster), 1, fd->Buffer))
    {
    ...
    while (byteCount > 0)
    {
        ...
        if (leftInBuffer == take)
        {
            ...
            if (fd->Public.Handle == ROOT_DIRECTORY_HANDLE)
            {
                ...
                // read next sector
                if (!DiskReadSector(disk, fd->CurrentCluster, 1, fd->Buffer))
                {
            ...
            else
            {
                ...
                // read next sector
                if (!DiskReadSector(disk, FAT_ClusterToLba(fd->CurrentCluster) + fd->CurrentSectorInCluster, 1, fd->Buffer))
                {
                    printf("FAT: read error!\n");
                    break;
                }
...
FAT_File* FAT_Open(DISK* disk, const char* path)
{
    ...
    while (*path) {
        ...
        bool isLast = false;
        const char* delim = strsearch(path, '/');
        if (delim != NULL)
        {
