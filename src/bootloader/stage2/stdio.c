...
void setCursor(int x, int y) {
    ...
    x86_outb(0x03D5, (u8)(pos & 0xFF));
    x86_outb(0x03D4, 0x0E);
    x86_outb(0x03D5, (u8)((pos >> 8) & 0xFF));
}
...
void printun(u64 num, int radix) {
    char buff[32];
    int pos = 0;
...
void printn(i64 number, int radix) {
    if(number < 0) {
        putChar('-');
...
{
    const u8 *u8Buffer = (const u8 *)buffer;
